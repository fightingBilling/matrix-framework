...
@Controller
public class ThreadInfoController extends MatrixRestController {
    @RequestMapping(value = "/monitor/threads", produces = "text/plain", method = RequestMethod.GET)
    @ResponseBody
    @Pass
    public String threadInfo(HttpServletRequest request) {
    ...
    private String toString(ThreadInfo threadInfo) {
        StringBuilder builder = new StringBuilder().append('\"').append(threadInfo.getThreadName()).append('\"').append(" Id=").append(threadInfo.getThreadId()).append(' ').append(
